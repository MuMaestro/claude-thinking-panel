...
def build_lines(events, width):
    """-> list of (text, attr_key, event_index|None). The index serves the click."""
    lines = []
    for i, ev in enumerate(events):
        ...
        if ev.expanded and ev.body:
            for para in ev.body.split("\n"):
                ...
                for w in wrapped:
                    lines.append(("    " + w, ev.kind + "_body", i))
        if ev.kind == "thinking":
            lines.append(("", "plain", None))
    return lines
...
def main(stdscr, watch_dir):
    ...
    while True:
        ...
        body_h = height - 1
        lines = build_lines(events, width)
        ...
        for y, (text, key, _idx) in enumerate(visible):
            try:
                stdscr.addstr(y, 0, text, attr_for(key))
            except curses.error:
                pass
        status = " click: expand · f: follow · e/c: expand/collapse · q: quit "
